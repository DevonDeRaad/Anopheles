...
spatial.autocorr<-c(spatial.autocorr,spatial.autocorr,spatial.autocorr)
spatial.autocorr<-abs(spatial.autocorr)
cors<-data.frame(env.var=rep(c("prec","hum","temp","rand","corr"), times=3),
                 test=c(rep("LFMM2", times=5), rep("BayeScEnv", times=5), rep("r2VIM", times=5)),
                 spat.autocorr=spatial.autocorr,
                 num.sig.outliers=out)

#4A
ggplot(cors, aes(x=spat.autocorr, y=num.sig.outliers, color=test)) +
...
genetic.autocorr<-c(genetic.autocorr,genetic.autocorr,genetic.autocorr)
cors$gen.autocorr<-abs(genetic.autocorr)

#make plot 4B
ggplot(cors, aes(x=gen.autocorr, y=num.sig.outliers, color=test)) +
  geom_point(cex=3, alpha=.6)+
  geom_smooth(method=lm, se=FALSE)+
  theme_classic()+
...
#4D
ggplot(cors, aes(x=var.type, y=num.sig.outliers)) + 
  geom_boxplot()+
...
t.test(cors$num.sig.outliers[cors$var.type == "environmental"], cors$num.sig.outliers[cors$var.type == "random"])
...
f<-data.frame(test=rep(test, times=3),
              r=rep(storer, times=3))

#create vector to hold the significant proportion for each of the tests in each of the genomic regions
prop.sig<-c()
test<-c(rep("LFMM2", times=20),rep("BayeScEnv", times=20),rep("r2VIM", times=20))
...
#4C
ggplot(f, aes(x=abs(r), y=prop.sig, color=comps)) + 
  geom_point(cex=3, alpha=.6)+
  geom_smooth(method=lm, se=FALSE)+
  theme_classic()+

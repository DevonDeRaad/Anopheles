...
lfmm.outlier.table <- read.csv("~/Downloads/ml.outputs/vetted.lfmm.prec.env.csv")
...
bayescan.outlier.table <- read.csv("~/Downloads/ml.outputs/vetted.bayescenv.prec.env.csv")
#add separate chrom and pos columns
...
r2vim.outlier.table <- read.csv("~/Downloads/ml.outputs/vetted.r2vim.prec.env.csv")[,2:3]
...
length(unique(lfmm.gene.list$gene)) #272

table(lfmm.gene.list$gene)[table(lfmm.gene.list$gene) >10]
hist(table(lfmm.gene.list$gene))
write.table(lfmm.gene.list, "/Users/devder/Desktop/anoph.phase2/overrep/lfmm.prec.gene.snp.match.txt", row.names = F, col.names = T, quote = F)
```
...
write.table(bayescan.gene.list, "/Users/devder/Desktop/anoph.phase2/overrep/bayescan.prec.gene.snp.match.txt", row.names = F, col.names = T, quote = F)
...
#write.table(unique(bayescan.gene.list$gene), "~/Downloads/bayescan.prec.gene.list.txt", row.names = F, col.names = F, quote = F)
write.table(r2vim.gene.list, "/Users/devder/Desktop/anoph.phase2/overrep/r2vim.prec.gene.snp.match.txt", row.names = F, col.names = T, quote = F)
#look into how many genes overlap between the methods
length(intersect(unique(lfmm.gene.list$gene),unique(bayescan.gene.list$gene)))
```
...
#there are 7 significantly enriched GO terms
write.table(result, file="/Users/devder/Desktop/anoph.phase2/overrep/lfmm.prec.go.overrep.txt",            row.names = F, col.names = F, quote = F)
...
#
write.table(result, file="/Users/devder/Desktop/anoph.phase2/overrep/bayescan.prec.go.overrep.txt", row.names = F, col.names = F, quote = F)
...
#
write.table(result, file="/Users/devder/Desktop/anoph.phase2/overrep/r2vim.prec.go.overrep.txt", row.names = F, col.names = F, quote = F)

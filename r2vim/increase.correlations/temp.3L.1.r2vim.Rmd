...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
              no.runs = 1, factor = 1, ntree = 6500, mtry.prop = .1,
...
runs<-data.frame(run1=r2vim$info$vim.run.1)
#write.csv(runs, "3L.temp.1.runs.csv", row.names = FALSE, quote = FALSE)
...
write.csv(df, "3L.temp.1.rel.vim.min.csv", row.names = FALSE, quote = FALSE)

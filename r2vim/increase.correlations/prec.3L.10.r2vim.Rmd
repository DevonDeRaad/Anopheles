...
#pos<-read.table("ag1000g.phase1.ar3.pass.biallelic.maf05.3L.geno.matrix.012.pos")
...
#snp.matrix<-read.table("ag1000g.phase1.ar3.pass.biallelic.maf05.3L.geno.matrix.012")[,-1]
#
...
#write.csv(pos, "3L.chrom.no.missing.pos", row.names = F, quote=FALSE)
#
...
#write.csv(reg_data_corrected,file="3L.precip.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
```{r}
#read in adjusted df
reg_data_corrected<-read.csv("3L.precip.corrected.csv")
p<-ncol(reg_data_corrected)-2
dim(reg_data_corrected)
reg_data_corrected[1:10,1:10]

#read in position info
pos<-read.csv("3L.chrom.no.missing.pos")
...
write.csv(df, "3L.prec.10.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

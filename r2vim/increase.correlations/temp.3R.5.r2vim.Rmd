...
#write.csv(reg_data_corrected,file="3R.temp.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
              no.runs = 2, factor = 1, ntree = 5000, mtry.prop = .2,
...
runs<-data.frame(run9=r2vim$info$vim.run.1,run10=r2vim$info$vim.run.2)
write.csv(runs, "3R.temp.5.runs.csv", row.names = FALSE, quote = FALSE)
...
df<-data.frame(chrom=pos$chrom,
               snp=pos$bp,
               vim.run.9=r2vim$info$vim.run.1,
               vim.run.10=r2vim$info$vim.run.2)
...
write.csv(df, "3R.temp.5.rel.vim.min.csv", row.names = FALSE, quote = FALSE)

...
#pos<-read.table("ag1000g.phase1.ar3.pass.biallelic.maf05.3R.geno.matrix.012.pos")
...
#snp.matrix<-read.table("ag1000g.phase1.ar3.pass.biallelic.maf05.3R.geno.matrix.012")[,-1]
#
...
##write out a copy of the pos dataframe with no missing data
#write.csv(pos, "3R.chrom.no.missing.pos", row.names = F, quote = FALSE)
#
##create reg_data including pop, precipitation variable, and X chrom SNP matrix
#reg_data<-as.data.frame(cbind(env.pop$population,env.pop$pannual,snp.matrix))
#
...
#write.csv(reg_data_corrected,file="3R.precip.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
```{r}
#read in adjusted df
reg_data_corrected<-read.csv("3R.precip.corrected.csv")
p<-ncol(reg_data_corrected)-2
dim(reg_data_corrected)
reg_data_corrected[1:10,1:10]

#read in position info
pos<-read.csv("3R.chrom.no.missing.pos")
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
              no.runs = 1, factor = 1, ntree = 15000, mtry.prop = 2*sqrt(p)/p,
              method = "ranger", type = "regression")

#make results df for manhattan plot
df<-data.frame(chrom=pos$chrom,
               snp=pos$bp,
               run.10=r2vim$info$vim.run.1)
#save rel.vim.min values
write.csv(df, "3R.prec.10.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

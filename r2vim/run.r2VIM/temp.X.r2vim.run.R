...
##read in adjusted df
reg_data_corrected<-read.csv("X.temp.corrected.csv")
p<-ncol(reg_data_corrected)-2

#read in position info
pos<-read.csv("X.chrom.no.missing.pos.csv")
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
              no.runs = 10, factor = 1, ntree = 2500, mtry.prop = 2*sqrt(p)/p,
...
write.csv(df, "X.temp.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

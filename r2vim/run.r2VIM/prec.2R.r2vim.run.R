...
##read in adjusted df
reg_data_corrected<-read.csv("2R.prec.corrected.csv")
p<-ncol(reg_data_corrected)-2

#read in position info
pos<-read.csv("2R.chrom.no.missing.pos.csv")
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
                     no.runs = 10, factor = 1, ntree = 5000, mtry.prop = 2*sqrt(p)/p,
...
write.csv(df, "2R.prec.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

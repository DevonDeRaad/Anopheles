...
##read in adjusted df
reg_data_corrected<-read.csv("X.prec.corrected.csv")
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
                     no.runs = 10, factor = 1, ntree = 3000, mtry.prop = sqrt(p)/p,
...
write.csv(df, "X.prec.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

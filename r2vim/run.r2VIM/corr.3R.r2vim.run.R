...
##read in adjusted df
reg_data_corrected<-read.csv("3R.corr.corrected.csv")
...
r2vim<-var.sel.r2vim(x = reg_data_corrected[,3:ncol(reg_data_corrected)], y=reg_data_corrected$response,
                     no.runs = 10, factor = 1, ntree = 1000, mtry.prop = 1/3,
...
write.csv(df, "3R.corr.rel.vim.min.csv", row.names=FALSE, quote=FALSE)

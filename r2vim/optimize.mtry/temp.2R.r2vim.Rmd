...
```{r}
detectCores()
#read in position information for 2R chrom
pos<-read.table("maf05.ld100.matrix.2R.012.pos")
...
snp.matrix<-read.table("maf05.ld100.matrix.2R.012")[,-1]
...
write.csv(pos, file = "2R.chrom.no.missing.pos.csv", row.names = FALSE, quote=FALSE)

#create reg_data including pop, temp variable, and X chrom SNP matrix
reg_data<-as.data.frame(cbind(env.pop$population,env.pop$t_mean,snp.matrix))
...
lines(results_optimization$ntree[results_optimization$mtry == p/3],results_optimization$PVE[results_optimization$mtry == p/3], ol="orange")
...
```{r}
##read in adjusted df
#reg_data_corrected<-read.csv("~/Downloads/2R.temp.corrected.csv")
#p<-ncol(reg_data_corrected)-2
#

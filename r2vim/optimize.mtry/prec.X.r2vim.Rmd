...
snp.matrix<-read.table("maf05.ld100.matrix.X.012")[,-1]

#drop all columns in the snp matrix containing missing data
snp.matrix<-snp.matrix[,colSums(is.na(snp.matrix)) == 0]

#create reg_data including pop, prec variable, and X chrom SNP matrix
...
write.csv(reg_data_corrected,file="X.prec.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
```{r}
##read in adjusted df
#reg_data_corrected<-read.csv("~/Downloads/X.prec.corrected.csv")
#p<-ncol(reg_data_corrected)-2
#

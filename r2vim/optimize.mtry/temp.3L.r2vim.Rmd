...
```{r}
detectCores()
#read in position information for 3L chrom
pos<-read.table("maf05.ld100.matrix.3L.012.pos")
...
write.csv(pos, file = "3L.chrom.no.missing.pos.csv", row.names = FALSE, quote=FALSE)
...
write.csv(reg_data_corrected,file="3L.temp.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
```{r}
##read in adjusted df
#reg_data_corrected<-read.csv("~/Downloads/3L.temp.corrected.csv")
#p<-ncol(reg_data_corrected)-2
#

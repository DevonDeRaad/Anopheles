...
```{r}
detectCores()
#read in position information for 2L chrom
pos<-read.table("maf05.ld100.matrix.2L.012.pos")
...
snp.matrix<-read.table("maf05.ld100.matrix.2L.012")[,-1]
...
write.csv(pos, file = "2L.chrom.no.missing.pos.csv", row.names = FALSE, quote=FALSE)
...
write.csv(reg_data_corrected,file="2L.temp.corrected.csv", row.names = FALSE, quote=FALSE)
```
...
```{r}
##read in adjusted df
#reg_data_corrected<-read.csv("~/Downloads/2L.temp.corrected.csv")
#p<-ncol(reg_data_corrected)-2
#

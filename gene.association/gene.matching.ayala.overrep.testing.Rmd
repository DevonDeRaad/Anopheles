---
title: "Identify gene associations, overrepresentation test"
author: "Devon DeRaad"
date: "4/13/2021"
...
#Match outlier SNP datasets with associated genes
```{r}
...
#split by test into 20 seperate dfs all contained in a list called 'X'
...
#now have gene.dfs, a list containing 20 separate dfs each containing the associated genes for a given GEA test

```

```{r}
#for each of the 20 datasets isolate 'gene' as a unique column using regular expressions
for (i in tests){
...
#calculate the number of unique genes associated with each outlier SNP dataset
j=1
for (i in tests){
  print(tests[j])
...
#Bring in a priori identified desiccation-tolerance genes from Ayala et al. (2019)
```{r}
...
#calculate the number of overlapping genes identified by each of our candidate gene datasets and whether that overlap is significant
```{r}
...
#plot the allele frequency patterns of outlier SNPs (from moisture associated GEA tests) found associated with the two top candidate genes from Ayala et al.
```{r}
...
prec.2578<-ggplot(df, aes(x=prec, y=frq, color=SNP)) + 
  ...
  theme_classic()+
  labs(x="Precipitation (mm/year)", y="Allele Frequency")
grid.arrange(prec.2578, hum.6026, nrow = 1)
